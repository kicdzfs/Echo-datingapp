...
const BottomTabBar = ({ activeTab, onChangeTab, onOpenPost }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md safe-bottom border-t border-gray-100">
      <div className="h-16 px-6 flex items-center justify-between relative">
        <div className="grid grid-cols-5 gap-6 w-full max-w-md mx-auto items-center">
          ...
          <div className="relative w-full h-full flex justify-center">
            <button
              onClick={onOpenPost}
              className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-14 h-14 bg-violet-600 rounded-full flex items-center justify-center shadow-lg hover:scale-105 transition-transform border-4 border-white"
            >
